...
from pywbemReq.cim_obj import CIMInstance, CIMInstanceName, CIMQualifier, \
    CIMProperty
from pywbemReq.cim_types import is_text
from pywbemReq.tupleparse import parse_classpath, parse_localclasspath, \
    parse_classname
import logging
...
def parse_localnamespacepath(parser, event, node):
    ...
    if not _is_start(next_event, next_node, 'NAMESPACE'):
        log.debug('{}{}'.format(next_event, next_node))
...
def parse_keyvalue(parser, event, node):
    ...
    if valuetype == 'string':
        ...
        if p == 'true':
            ...
            raise ParseError('invalid boolean value "%s"' % repr(p))
